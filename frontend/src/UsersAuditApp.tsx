...
export default function UsersAuditApp() {
  ...
  const load = async () => {
    try {
      const [rolesResponse, usersResponse, auditResponse] = await Promise.all([
        fetch(`${apiBaseUrl}/api/v1/administration/roles`, { headers: auth() }),
        fetch(`${apiBaseUrl}/api/v1/administration/users`, { headers: auth() }),
        fetch(`${apiBaseUrl}/api/v1/audit?limit=100`, { headers: auth() }),
      ])
      if (!rolesResponse.ok || !usersResponse.ok || !auditResponse.ok) throw new Error('No fue posible cargar la administración.')
      setRoles(await rolesResponse.json()); setUsers(await usersResponse.json()); setAudit(await auditResponse.json())
  ...
  const save = async () => {
    const url = editing ? `${apiBaseUrl}/api/v1/administration/users/${editing.id}` : `${apiBaseUrl}/api/v1/administration/users`
    const body = editing ? { firstName: form.firstName, lastName: form.lastName, email: form.email, roleIds: form.roleIds } : form
    const response = await fetch(url, { method: editing ? 'PUT' : 'POST', headers: auth(), body: JSON.stringify(body) })
  ...
  const changeStatus = async (user: User) => {
    const response = await fetch(`${apiBaseUrl}/api/v1/administration/users/${user.id}/status`, { method: 'POST', headers: auth(), body: JSON.stringify({ active: !user.isActive }) })
    if (!response.ok) { const data = await response.json().catch(() => ({})); setError(data.message ?? 'No fue posible cambiar el estado.'); return }
    await load()
  }

  const unlock = async (id: string) => { await fetch(`${apiBaseUrl}/api/v1/administration/users/${id}/unlock`, { method: 'POST', headers: auth() }); await load() }
  const resetPassword = async (id: string) => { const password = window.prompt('Nueva contraseña (mínimo 8 caracteres)'); if (!password) return; await fetch(`${apiBaseUrl}/api/v1/administration/users/${id}/password`, { method: 'POST', headers: auth(), body: JSON.stringify({ password }) }); await load() }
